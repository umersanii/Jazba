...
import jazba.models.User;
import jazba.models.UserSession;
...
public class MainSceneController {
    ...
    @FXML
    public void navigateToCreateWorkout(ActionEvent event)
    {
        try {
                    SceneManager switchScene = new SceneManager();
                    switchScene.switchScene("/jazba/view/user/CreateWorkout.fxml");
                } catch (Exception e) {
                    e.printStackTrace();
                }
    }
    ...
    @FXML
    public void navigateToLogWorkout(ActionEvent event)
    {
        try {
                    SceneManager switchScene = new SceneManager();
                    switchScene.switchScene("/jazba/view/user/LogWorkout.fxml");
                } catch (Exception e) {
                    e.printStackTrace();
                }
    }
    ...
    private void updateAchievements(int memberId) {
        try {
            ...
            for (int i = 1; i <= 12; i++) {
                boolean isUnlocked = achievementDAO.isAchievementUnlocked(1, "badge" + i); // Check if achievement is unlocked
                ...
                if (isUnlocked) {
                    ...
                    try{
                        badge.setEffect(colorAdjust);  // Change background color to black if unlocked
...
public void showNotifications(MouseEvent event) {
    ...
    // Example notifications with type and timestamp
    UserDAO userSession = new UserDAO();
    int user = UserSession.getLoggedInUserID();
    List<Notification> notifications = userSession.getLatestNotifications(user); // Create a VBox to hold the notifications
    VBox notificationBox = new VBox(10);  // Spacing between notifications
    notificationBox.setStyle("-fx-background-color: black; -fx-padding: 10;");

    // Loop through the notifications and create a styled box for each
    for (Notification notification : notifications) {
        HBox notificationItem = new HBox(10);  // Space between elements in each notification box
        notificationItem.setStyle("-fx-background-color: #333; -fx-padding: 10; -fx-border-radius: 5px; -fx-background-radius: 5px;");
        notificationItem.setAlignment(Pos.CENTER_LEFT);

        // Set type (e.g., achievement, transaction) in bold
        Label typeLabel = new Label(notification.getType());
        typeLabel.setStyle("-fx-font-weight: bold; -fx-text-fill: #FFD700; -fx-font-size: 14px;");
        
        // Set the timestamp
        Text timestampText = new Text(notification.getTimestamp());
        timestampText.setFill(Color.GRAY);
        timestampText.setFont(Font.font(12));

        // Set the message
        Label messageLabel = new Label(notification.getMessage());
        messageLabel.setStyle("-fx-text-fill: white; -fx-font-size: 14px;");

        // Add the type, timestamp, and message to the HBox
        notificationItem.getChildren().addAll(typeLabel, timestampText, messageLabel);

        // Add the notification item to the VBox
    ...
    // Install the tooltip to the source node
    Tooltip.install(source, tooltip);

    // Set the tooltip's content display to hold the VBox
    tooltip.setGraphic(notificationBox);
    ...
    // <Button fx:id="overviewButton" alignment="CENTER" mnemonicParsing="false" prefHeight="40.0" prefWidth="130.0" styleClass="nav-button" text="Overview" onAction="#navigateToOverview" />
    // <Button fx:id="logsButton" mnemonicParsing="false" prefHeight="40.0" prefWidth="150.0" styleClass="nav-button" text="Logs" onAction="#navigateTologsButton"/>
    // <Button fx:id="achievementsButton" mnemonicParsing="false" prefHeight="40.0" prefWidth="150.0" styleClass="nav-button" text="Achievements" onAction="#navigateToAchievements"/>
    // <Button fx:id="statsButton" mnemonicParsing="false" onAction="#navigateToStatsPage" prefHeight="40.0" prefWidth="130.0" styleClass="nav-button" text="Stats" onAction="#navigateToStats"/>
    @FXML
    public void navigateToCreate(ActionEvent event) {
        navigateTo("/jazba/view/user/CreateWorkout.fxml");
    }
    
    @FXML
    public void navigateToOverview(ActionEvent event) {
        navigateTo("/jazba/view/user/MainScene.fxml");
    }
    
    @FXML
    public void navigateToLogs(ActionEvent event) {
        navigateTo("/jazba/view/user/LogWorkout.fxml");
    }
    
    @FXML
    public void navigateToStatsPage(ActionEvent event) {
        navigateTo("/jazba/view/user/StatsPage.fxml");
    }
    
    private void navigateTo(String path) {
        try {
            SceneManager switchScene = new SceneManager();
            switchScene.switchScene(path);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
